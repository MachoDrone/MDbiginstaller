...
echo "Version: 0.00.16 - 11:05:00 23JUL25 🚀"
...
pid=\$!
container_id=\$(docker ps -q -l)  # Last created container
for i in {1..20}; do
  sleep 1
  if docker logs \$container_id 2>&1 | grep -q "Creating new keypair"; then
    docker stop \$container_id || true
    break
  fi
done
# Force kill if still running
if [ -n "\$(docker ps -q -l)" ]; then
