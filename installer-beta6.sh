...
echo "Version: 0.00.11 - 09:42:00 23JUL25 🚀"
...
pid=\$!
sleep 3 && if [ -n "\$(docker ps -q -f "ancestor=nosana/nosana-cli:latest")" ]; then docker stop \$(docker ps -q -f "ancestor=nosana/nosana-cli:latest"); fi || true
